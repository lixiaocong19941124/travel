...
<div id="content">
   ...
   <div id="outerdiv" style="z-index: 1003;position:fixed;top:0;left:0;width:100%;height:100%;display:none;">
      <figure>
         <figcaption style="z-index: 99">
            <h3 style="color: white"></h3>
            <p style="color: white"></p>
            <span style="display: inline-block;position: absolute;top: 10px;right: 10px;height: 40px;width: 40px;background: url('../image/imgs-grid-icons.png') no-repeat 100%;background-position: -98px -12px;cursor: pointer"></span>
         </figcaption>
         <img src="" alt="" style="margin: 0px auto; height: 540px; width: auto;max-width: 100%;">
      </figure>
       <div id="leftnav" onclick="f_clnav(this)"></div>
       <div id="rightnav" onclick="f_clnav(this)"></div>
   </div>
</div>
...
<script>
   ...
   function search() {
      if($('.sousuo').val()==''){
         ...
         $.ajax({
            type: "post",
            url: G_url + 'views/tvup?s='+$('.sousuo').val(),
            dataType: "json",
            async:false,
            success: function (res) {
               if (res.ok) {
   ...
   //点击图片显示左右方向导航，并且将图片的上一个或下一个加载出来
   function fdtp(b) {
       if(b == sjdata.length -1){
           $('#leftnav').show();
           $('#rightnav').hide();
       }else if(b == 0){
           $('#rightnav').show();
        ...
        $('figcaption h3').html(sjdata[b].bt);
        $('figcaption p').html(sjdata[b].js);
        $('figure img').attr('src',sjdata[b].tp);
      $('#outerdiv').fadeIn();
   }
    //计算显示图片的位置
   function f_clnav(a) {
       var dtindex=$(a).attr('id');
       switch (dtindex) {//计算id的值
           case 'leftnav'://计算常量
               zhizhen--;
               fdtp(zhizhen);
               break;
           case 'rightnav'://计算数字相加
               zhizhen++;
               fdtp(zhizhen);
               break;
       }
